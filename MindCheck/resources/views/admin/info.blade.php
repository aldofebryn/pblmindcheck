...
<div class="max-w-4xl mx-auto space-y-6">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
        <div>
            <h1 class="text-2xl font-bold text-slate-800">Informasi Sistem</h1>
            <p class="text-slate-500 text-sm mt-1">Spesifikasi instrumen, parameter klasifikasi, dan detail sistem MindCheck.</p>
        </div>
    </div>
    ...
    <div class="bg-white/70 backdrop-blur-md border border-white/60 rounded-2xl shadow-sm overflow-hidden">
        <div class="px-6 py-4 border-b border-slate-100 flex items-center gap-3">
            ...
            <div>
                <h3 class="font-bold text-slate-800 text-base">Detail Aplikasi</h3>
                <p class="text-slate-400 text-xs mt-0.5">Spesifikasi dasar sistem skrining</p>
            </div>
        </div>
        <div class="divide-y divide-slate-50">
            ...
            <div class="flex flex-col sm:flex-row gap-2 sm:gap-6 px-6 py-4 border-b border-slate-50 last:border-0">
                <span class="text-slate-400 font-medium w-48 shrink-0">{{ $k }}</span>
        ...
        <div class="divide-y divide-slate-50">
            ...
            <div class="px-6 py-4 flex flex-col sm:flex-row sm:items-center gap-3">
                ...
                <div class="flex flex-wrap gap-2">
                    ...
                    <span class="text-xs font-semibold px-3 py-1.5 rounded-lg bg-{{ $c }}-50/80 text-{{ $c }}-700 border border-{{ $c }}-200/50 whitespace-nowrap">
                        {{ $kat }}<span class="ml-1.5 opacity-60">{{ $range }}</span>
                    </span>
                    @endforeach
                </div>
            </div>
    ...
    <div class="bg-white/70 backdrop-blur-md border border-white/60 rounded-2xl shadow-sm overflow-hidden">
        <div class="px-6 py-4 border-b border-slate-100 flex items-center gap-3">
            ...
            <div>
                ...
                <p class="text-slate-400 text-xs mt-0.5">Aturan rujukan berdasarkan tingkat keparahan</p>
            </div>
        </div>
        <div class="divide-y divide-slate-50">
            @foreach([
            ...
            <div class="flex items-start gap-4 px-6 py-4">
                ...
                <div>
                    ...
                    <p class="text-xs text-slate-400 mt-1">→ {{ $action }}</p>
                </div>
            </div>
            @endforeach
        </div>
    ...
    {{-- ── Disclaimer ───────────────────────────────────────────── --}}
    <div class="bg-amber-50/80 border border-amber-200/70 rounded-2xl px-6 py-4 flex gap-3 items-start shadow-sm">
